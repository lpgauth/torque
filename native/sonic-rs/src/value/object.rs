...
impl Object {
    ...
    // Torque patch: last-match lookup for parsed objects that may contain
    // duplicate keys. Parsed objects keep entries in document order, so a
    // reverse scan yields the last occurrence; owned (hash-backed) objects hold
    // unique keys and fall back to `get`.
    #[inline]
    pub fn get_last<Q: AsRef<str>>(&self, key: &Q) -> Option<&Value> {
        let key = key.as_ref();
        match self.0.as_ref2() {
            ValueRefInner::Object(pairs) => pairs
                .iter()
                .rev()
                .find(|(k, _)| k.as_str() == Some(key))
                .map(|(_, v)| v),
            _ => self.0.get_key(key),
        }
    }
